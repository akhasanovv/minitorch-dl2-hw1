...
def test_sigmoid(a: float) -> None:
    ...
    * It is  strictly increasing.
    """
    assert lt(sigmoid(a), 1.0 + 1e-12)
    assert lt(0.0 - 1e-12, sigmoid(a))
    
    assert_close(sigmoid(-a), 1 - sigmoid(a))
    assert_close(sigmoid(0), 0.5)

    assert lt(sigmoid(1.0 - 0.1), sigmoid(1.0))
...
def test_transitive(a: float, b: float, c: float) -> None:
    ...
    first_bit = lt(a, b) * lt(b, c)
    second_bit = lt(a, b)
    assert lt(first_bit - 1.0, second_bit)
