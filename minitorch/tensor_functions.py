...
class Sigmoid(Function):
    ...
    def backward(ctx: Context, grad_output: Tensor) -> Tensor:
        ...
        s = sigm.f.mul_zip(sigm, one.f.add_zip(one, sigm.f.neg_map(sigm)))
...
class Permute(Function):
    ...
    def forward(ctx: Context, a: Tensor, order: Tensor) -> Tensor:
        order_copy = [int(order[i]) for i in range(order.size)]
        ctx.save_for_backward(order_copy)
        return a._new(a._tensor.permute(*order_copy))
